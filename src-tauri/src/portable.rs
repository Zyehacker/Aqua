...
fn root(mc_dir: Option<String>) -> Result<PathBuf, String> {
    mc_dir.map(PathBuf::from).or_else(default_mc_dir).ok_or_else(|| "Could not determine Aqua directory".into())
}
...
fn collect_files(dir: &Path, base: &Path, out: &mut Vec<(PathBuf, PackageFile)>) -> Result<(), String> {
    for entry in fs::read_dir(dir).map_err(|e| e.to_string())? {
        ...
        if path.is_dir() {
            ...
            let relative = path.strip_prefix(base).map_err(|e| e.to_string())?.to_path_buf();
            out.push((path, PackageFile { path: relative.to_string_lossy().replace('\\', "/"), size, sha1 }));
        }
    }
    Ok(())
}
...
#[tauri::command]
pub fn export_instance(instance_id: String, destination: String, mc_dir: Option<String>) -> Result<String, String> {
    let root = root(mc_dir)?;
    let metadata = read_metadata(&root, &instance_id).ok_or_else(|| format!("Instance not found: {instance_id}"))?;
    let instance_root = instance_dir(&root, &instance_id);
    let package = PathBuf::from(destination);
    if let Some(parent) = package.parent() { fs::create_dir_all(parent).map_err(|e| e.to_string())?; }

    let mut files = Vec::new();
    if instance_root.exists() {
        let before = files.len();
        collect_files(&instance_root, &instance_root, &mut files)?;
        for (_, entry) in files.iter_mut().skip(before) { entry.path = format!("instance/{}", entry.path); }
    }
    ...
    writer.write_all(serde_json::to_string_pretty(&manifest).map_err(|e| e.to_string())?.as_bytes()).map_err(|e| e.to_string())?;
    for (source, entry) in files {
        if entry.path.eq_ignore_ascii_case("account.json") || entry.path.to_ascii_lowercase().contains("credentials") || entry.path.to_ascii_lowercase().contains("token") {
...
fn unpack_and_validate(package_path: &str, temp: &Path) -> Result<PackageManifest, String> {
    ...
    let mut archive = ZipArchive::new(file).map_err(|e| format!("Invalid .aquainst archive: {e}"))?;
    let mut manifest_file = archive.by_name("manifest.json").map_err(|_| "Package is missing manifest.json".to_string())?;
    let mut raw = String::new();
    manifest_file.read_to_string(&mut raw).map_err(|e| e.to_string())?;
    drop(manifest_file);
    let manifest: PackageManifest = serde_json::from_str(&raw).map_err(|e| format!("Invalid package manifest: {e}"))?;
    ...
    for index in 0..archive.len() {
        ...
        let Some(path) = entry.name().strip_prefix("files/") else { continue; };
    ...
    for expected in &manifest.files {
        if expected.path.eq_ignore_ascii_case("account.json") || expected.path.to_ascii_lowercase().contains("credentials") || expected.path.to_ascii_lowercase().contains("token") {
            return Err("Package contains forbidden authentication data".to_string());
        }
        let extracted = temp.join(safe_relative(&expected.path)?);
        if !extracted.exists() { return Err(format!("Package is missing declared file: {}", expected.path)); }
        let (size, sha1) = hash_file(&extracted)?;
        if size != expected.size || sha1 != expected.sha1 { return Err(format!("Integrity check failed for {}", expected.path)); }
    }
    Ok(manifest)
}

#[tauri::command]
pub async fn import_instance(app: AppHandle, package_path: String, mc_dir: Option<String>, requested_name: Option<String>) -> Result<String, String> {
    let root = root(mc_dir)?;
    let manifest_preview = {
        ...
        let mut raw = String::new();
        manifest_file.read_to_string(&mut raw).map_err(|e| e.to_string())?;
        serde_json::from_str::<PackageManifest>(&raw).map_err(|e| format!("Invalid package manifest: {e}"))?
    };
    if manifest_preview.format != "aqua-instance" || manifest_preview.format_version > FORMAT_VERSION { return Err("Unsupported Aqua instance package version".into()); }
    let name = requested_name.filter(|value| !value.trim().is_empty()).unwrap_or_else(|| manifest_preview.instance.name.clone());
    ...
    let temp = root.join(".aquainst-import").join(format!("{}-{}", id, std::process::id()));
    ...
    let result: Result<(), String> = {
        ...
        if source_instance.exists() { copy_dir(&source_instance, &target)?; }
...
pub(crate) fn copy_dir(source: &Path, target: &Path) -> Result<(), String> {
    ...
    for entry in fs::read_dir(source).map_err(|e| e.to_string())? {
        ...
        if entry.path().is_dir() { copy_dir(&entry.path(), &destination)?; }
