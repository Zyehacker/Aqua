...
fn archive_mod_identity(path: &Path, loader: &str) -> Result<Option<(String, String)>, String> {
    ...
    match loader.trim().to_ascii_lowercase().as_str() {
        "fabric" => {
            let Some(raw) = archive_text(&mut archive, "fabric.mod.json") else {
                return Ok(None);
            };
            let metadata: serde_json::Value =
                serde_json::from_str(&raw).map_err(|e| format!("Invalid fabric.mod.json: {e}"))?;
        ...
        "forge" => {
            ...
            for index in 0..archive.len() {
                if archive
                    .by_index(index)
                    .ok()
                    .map(|entry| entry.name().ends_with("mods.toml"))
                    .unwrap_or(false)
                {
                    let name = archive
                        .by_index(index)
                    ...
                    if let Some(name) = name {
                        raw = archive_text(&mut archive, &name);
                    }
                    break;
                }
            }
            ...
            Ok(field("modId").map(|id| {
                (
                    id,
                    field("version").unwrap_or_else(|| "unknown".to_string()),
                )
            }))
        }
        _ => Ok(None),
...
fn compatible_version_for(
    ...
    let game_ok = version["game_versions"]
        ...
        .map(|values| {
            values
                .iter()
                .any(|value| value.as_str() == Some(mc_version))
        })
        .unwrap_or(false);
    if !game_ok {
    ...
    if checks_loader {
        ...
        let loader_ok = loader_key.is_empty()
            || loader_key == "vanilla"
            || version["loaders"]
                .as_array()
                .map(|values| {
                    values.iter().any(|value| {
                        value
                            .as_str()
                            .map(|value| value.eq_ignore_ascii_case(&loader_key))
                            .unwrap_or(false)
                    })
                })
                .unwrap_or(false);
...
fn sanitize_download_filename(filename: &str) -> String {
    ...
    let resolved =
        if cleaned.ends_with(".jar") || cleaned.ends_with(".zip") || cleaned.ends_with(".mrpack") {
            cleaned.to_string()
...
fn safe_replace_file(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    if destination.exists() {
        ...
        fs::remove_file(destination).map_err(|e| {
            format!(
                "Unable to remove existing destination {}: {e}",
                destination.display()
            )
        })?;
    }
...
async fn resolve_project(
    ...
    if !visiting.insert(project_id.to_string()) {
        return Err(format!(
            "Dependency conflict: cyclic dependency involving {project_id}"
        ));
    }
    ...
    for version in versions {
        let (compatible, reason) =
            compatible_version_for(&version, mc_version, loader, project_type);
        append_launcher_log(
            "info",
        ...
        if let Some((filename, url)) =
            version_file(&version).map(|(filename, url)| (filename.to_string(), url.to_string()))
        {
            if compatible {
    ...
    candidates.sort_by(|a, b| {
        ...
        b_release.cmp(&a_release).then_with(|| {
            b.0["date_published"]
                .as_str()
                .cmp(&a.0["date_published"].as_str())
        })
    });
            ...
            if dependency_type == "incompatible" {
                if let Some(project) = dependency_project {
                    if resolved.contains_key(project) || installed.contains(project) {
                        return Err(format!(
                            "Dependency conflict: {project} is incompatible with {project_id}"
                        ));
                    }
                }
                continue;
            }
            let Some(project) = dependency_project else {
                continue;
            };
            if dependency_type == "optional" {
                continue;
            }
            Box::pin(resolve_project(
                client,
                project,
                mc_version,
                loader,
...
fn read_dir_items(dir: &Path) -> Vec<LocalItem> {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_file() {
                if entry.file_name().to_string_lossy() == ".aqua-modrinth.json"
                    || entry
                        .file_name()
                        .to_string_lossy()
                        .starts_with(".aqua-download-")
                {
                    continue;
                }
...
pub async fn search_modrinth(
    ...
    {
        let root = mc_root.as_ref().ok_or_else(|| {
            "Instance metadata unavailable: Minecraft directory is not configured".to_string()
        })?;
        let metadata = read_metadata(root, instance_id).ok_or_else(|| {
            format!(
                "Instance '{}' not found. The instance metadata may have been deleted or moved.",
                instance_id
            )
        })?;
        (
    ...
    if !resolved_loader.trim().is_empty()
        && !matches!(resolved_loader.as_str(), "vanilla" | "fabric" | "forge")
    {
    ...
    if project_type_for(&category) == "datapack"
        ...
        && resp["hits"].as_array().map(|hits| hits.is_empty()).unwrap_or(true)
    {
        let mut fallback = Url::parse("https://api.modrinth.com/v2/search").map_err(|e| e.to_string())?;
        let facets = serde_json::to_string(&vec![vec!["project_type:datapack"]])
            .map_err(|e| e.to_string())?;
        fallback.query_pairs_mut()
            .append_pair("query", &query)
            .append_pair("limit", &limit.unwrap_or(24).to_string())
    ...
    for hit in hits {
        ...
        minecraft_matches += project_versions
            ...
            .count();
        loader_matches += project_versions
            .iter()
            .filter(|version| {
                version["loaders"]
                    .as_array()
                    .map(|values| {
                        values.iter().any(|value| {
                            value
                                .as_str()
                                .map(|value| value.eq_ignore_ascii_case(&resolved_loader))
                                .unwrap_or(false)
                        })
                    })
                    .unwrap_or(false)
            ...
            .count();
        compatible_versions += project_versions
            .iter()
            .filter(|version| {
                compatible_version_for(
                    version,
                    &resolved_mc_version,
                    &resolved_loader,
                    &wanted_type,
                )
                .0 && version_file(version).is_some()
            })
            .count();
        let resolved_version = project_versions.iter().find(|version| {
            compatible_version_for(
                version,
                &resolved_mc_version,
                &resolved_loader,
        ...
        if resolved_version.is_none() {
            ...
            let has_mc_match = project_versions.iter().any(|v| {
                v["game_versions"]
                    .as_array()
                    .map(|values| {
                        values
            ...
            let has_loader_match = !matches!(wanted_type.as_str(), "mod" | "modpack")
                || project_versions.iter().any(|v| {
                    v["loaders"]
                        .as_array()
                        .map(|values| {
                            values.iter().any(|value| {
        ...
        let resolved_version_id = resolved_version
        ...
        if resolved_version_id.is_none() {
            append_launcher_log(
                ...
                &format!(
                    "instance_id={} project_id={} issue=missing_version_id",
                    instance_id.as_deref().unwrap_or("none"),
                    id
                ),
            );
        ...
        out.push(ModSearchResult {
            ...
            compatibility_reason: format!(
                "Compatible with Minecraft {} and {}",
                resolved_mc_version, resolved_loader
            ),
            resolved_version_id,
        });
        append_launcher_log(
...
pub fn remove_instance_item(
    ...
    let instance_id =
        instance_id.ok_or_else(|| "An instance ID is required to remove content.".to_string())?;
    let category =
        category.ok_or_else(|| "A content category is required to remove content.".to_string())?;
...
pub async fn install_modrinth_project(
    ...
    if category == "modpacks" {
        return install_modrinth_modpack(
            app,
            project_id,
            mc_version,
            mc_dir,
            requested_name.unwrap_or_else(|| "Imported Modpack".into()),
            icon_url,
        )
        .await;
    }
    ...
    if metadata.mc_version != mc_version {
        return Err(format!(
            "Instance Minecraft version {} does not match requested version {}",
            metadata.mc_version, mc_version
        ));
    }
    if metadata.loader != loader {
        return Err(format!(
            "Instance loader {} does not match requested loader {}",
    ...
    let instance = metadata
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| instance_dir(&mc_dir, &instance_id));
    let folder = if category == "data-packs" || category == "datapacks" {
        let world = world_path
            .map(PathBuf::from)
            .ok_or_else(|| "Data packs require a selected Minecraft world.".to_string())?;
        if !world.is_dir() || !world.join("level.dat").is_file() {
            return Err("Data packs require a valid Minecraft world containing level.dat.".to_string());
        }
        let saves_root = instance.join("saves");
        let canonical_world = fs::canonicalize(&world)
            .map_err(|error| format!("Unable to resolve selected Minecraft world: {error}"))?;
        let canonical_saves = fs::canonicalize(&saves_root)
            .map_err(|_| "Selected world is not inside this instance's saves directory.".to_string())?;
        if !canonical_world.starts_with(&canonical_saves) {
            return Err("Selected world is not inside this instance's saves directory.".to_string());
        }
        canonical_world.join("datapacks")
    } else {
        instance.join(folder_for(&category))
    };
    fs::create_dir_all(&folder).map_err(|e| e.to_string())?;
    ...
    let mut installed_by_project = installed
        .iter()
        .map(|item| (item.project_id.clone(), item.clone()))
        .collect::<HashMap<_, _>>();
    ...
    for file in resolved.values() {
        ...
        if let Some(previous) = installed_by_project.get(&file.project_id) {
            ...
            if previous_path.exists() && previous_path != destination {
                fs::remove_file(previous_path)
                    .map_err(|e| format!("Unable to replace installed dependency: {e}"))?;
            }
        }
        ...
        loop {
            ...
            match job {
                Some(job) => {
                    ...
                    if matches!(job.status.as_str(), "failed" | "cancelled") {
                        return Err(job
                            .error
                            .clone()
                            .unwrap_or_else(|| format!("Download failed for {}", safe_filename)));
                    }
                ...
                None => {
                    return Err(format!(
                        "Download job {} was not created for {}",
            ...
            if let Ok(entries) = fs::read_dir(&folder) {
                for entry in entries.flatten() {
                    ...
                    if existing == destination
                        || existing.extension().and_then(|value| value.to_str()) != Some("jar")
                    {
                        continue;
                    }
                    if archive_mod_identity(&existing, &loader)?
                        .map(|(id, _)| id == resolved_id)
                        .unwrap_or(false)
                    {
                        fs::remove_file(existing)
                            .map_err(|e| format!("Unable to replace conflicting mod: {e}"))?;
                    }
                }
            }
        }
    ...
    requested_destination
        .ok_or_else(|| "Install plan did not contain the requested project".to_string())
}
...
pub async fn install_modrinth_modpack(
            ...
            items.iter().find(|item| {
                item["game_versions"]
                    ...
                    .map(|values| {
                        values
                            .iter()
                            .any(|value| value.as_str() == Some(mc_version.as_str()))
                    })
                    .unwrap_or(false)
                    && item["files"]
                        .as_array()
                        .and_then(|files| files.first())
                        .and_then(|file| file["filename"].as_str())
                        .map(|name| name.ends_with(".mrpack"))
                        .unwrap_or(false)
            })
        })
    ...
    let loaders = version["loaders"]
        .as_array()
        .ok_or("Modpack does not declare a loader")?;
    let loader = if loaders.iter().any(|item| item.as_str() == Some("fabric")) {
        "fabric"
    } else if loaders.iter().any(|item| item.as_str() == Some("forge")) {
        "forge"
    } else {
        return Err("This modpack requires a loader Aqua cannot install yet.".into());
    };
    let dependencies = version["dependencies"]
        .as_object()
        .ok_or("Modpack does not declare dependencies")?;
    let loader_version = dependencies
        .get(if loader == "fabric" {
            "fabric-loader"
        } else {
            "forge"
        })
        .and_then(|value| value.as_str())
        .map(String::from);
    let pack_url = version["files"]
        .as_array()
        .and_then(|files| files.iter().find_map(|file| file["url"].as_str()))
        .ok_or("Modpack download URL missing")?;
    ...
    let (installed_version_id, resolved_loader) = crate::install::install_version(
        app.clone(),
        loader.to_string(),
        mc_version.clone(),
        loader_version,
    ...
    let mut archive = zip::ZipArchive::new(Cursor::new(pack_bytes))
        .map_err(|e| format!("Invalid modpack archive: {e}"))?;
    let index: serde_json::Value = {
        let mut file = archive
            .by_name("modrinth.index.json")
            .map_err(|_| "Modpack is missing modrinth.index.json")?;
        let mut raw = String::new();
    ...
    let files = index["files"]
        .as_array()
        .ok_or("Modpack index has no files")?;
    for item in files {
        ...
        let downloads = item["downloads"]
            ...
            .and_then(|value| value.as_str())
            .ok_or_else(|| format!("No download URL for {path}"))?;
    ...
    for prefix in ["overrides", "client-overrides"] {
        for index in 0..archive.len() {
            ...
            let Some(path) = entry.name().strip_prefix(&format!("{prefix}/")) else {
                continue;
            };
            let destination = instance.join(path);
            if entry.is_dir() {
    ...
    let java_path = {
        ...
        let metadata = fs::read_to_string(version_json)
            .ok()
    ...
    let java_version = java_path
        .as_deref()
        .and_then(|path| crate::java::check_java_runtime(Path::new(path), Some(&mc_version)))
...
mod tests {
    use super::{
        compatible_version, compatible_version_for, normalize_loader, version_file,
        version_satisfies,
    };
    ...
    fn non_mod_content_uses_minecraft_version_without_loader_requirement() {
        ...
        for project_type in ["resourcepack", "datapack", "shader"] {
            assert_eq!(
                compatible_version_for(&version, "26.2", "fabric", project_type),
                (true, "metadata_match")
            );
        }
        assert_eq!(
            compatible_version_for(&version, "26.1", "fabric", "datapack").1,
    ...
    fn version_file_prefers_primary_download() {
        ...
        assert_eq!(
            ...
            Some(("mod.jar", "https://example.invalid/mod.jar"))
        );
    }
    ...
    fn version_file_prefers_primary_installable_jar() {
        ...
        assert_eq!(
            ...
            Some((
                "fabric-api-1.0.jar",
                "https://example.invalid/fabric-api-1.0.jar"
            ))
        );
    }
}
